...
class PINNNet(nn.Module):
    def __init__(self, inputNode=2, hiddenNode=256, outputNode=1):
        ...
        self.T_dotr = nn.Parameter(torch.normal(torch.tensor(0.0), torch.tensor(1.0), ), requires_grad=True)
        self.T_U_dotr = nn.Parameter(torch.normal(torch.tensor(0.0), torch.tensor(1.0), ), requires_grad=True)
        self.T_U2_dotr = nn.Parameter(torch.normal(torch.tensor(0.0), torch.tensor(1.0), ), requires_grad=True)
        self.T_U3_dotr = nn.Parameter(torch.normal(torch.tensor(0.0), torch.tensor(1.0), ), requires_grad=True)
        self.K_delta = nn.Parameter(torch.normal(torch.tensor(0.0), torch.tensor(1.0), ), requires_grad=True)
        self.K_U_delta = nn.Parameter(torch.normal(torch.tensor(0.0), torch.tensor(1.0), ), requires_grad=True)
        self.K_U2_delta = nn.Parameter(torch.normal(torch.tensor(0.0), torch.tensor(1.0), ), requires_grad=True)
        self.K_U3_delta = nn.Parameter(torch.normal(torch.tensor(0.0), torch.tensor(1.0), ), requires_grad=True)
        self.N_r = nn.Parameter(torch.normal(torch.tensor(0.0), torch.tensor(1.0), ), requires_grad=True)
        self.N_r3 = nn.Parameter(torch.normal(torch.tensor(0.0), torch.tensor(1.0), ), requires_grad=True)
        self.N_U_r3 = nn.Parameter(torch.normal(torch.tensor(0.0), torch.tensor(1.0), ), requires_grad=True)
        self.N_U2_r3 = nn.Parameter(torch.normal(torch.tensor(0.0), torch.tensor(1.0), ), requires_grad=True)
        self.N_U3_r3 = nn.Parameter(torch.normal(torch.tensor(0.0), torch.tensor(1.0), ), requires_grad=True)
    ...
    # unknown parameters
    def pinn_loss(self, r_true, r_pred, delta, U, r_prv):
        sampling_time = 1
        assert r_true.shape == r_pred.shape == delta.shape == U.shape
        F_rudder = self.K_delta * delta + self.K_U_delta * U * delta + self.K_U2_delta * U * U * delta + self.K_U3_delta * U * U * U * delta
        F_hydro = self.N_r * r_pred + self.N_r3 * r_pred * r_pred * r_pred + self.N_U_r3 * U * r_pred * r_pred * r_pred + self.N_U2_r3 * U * U * r_pred * r_pred * r_pred + self.N_U3_r3 * U * U * U * r_pred * r_pred * r_pred
        r_dot = (r_pred - r_prv) / sampling_time
        R = F_rudder - F_hydro - (self.T_dotr + self.T_U_dotr * U + self.T_U2_dotr * U * U + self.T_U3_dotr * U * U * U) * r_dot
